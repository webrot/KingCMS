...
namespace King.Environment.Commands
{
    public abstract class DefaultCommandHandler : ICommandHandler
    {
        ...
        private void SetSwitchValue(KeyValuePair<string, string> commandSwitch)
        {
            ...
            if (!propertyInfo.GetCustomAttributes(typeof(KingSwitchAttribute), false).Any())
            {
                throw new InvalidOperationException(T["A property \"{0}\" exists but is not decorated with \"{1}\"", commandSwitch.Key, typeof(KingSwitchAttribute).Name]);
            }
        ...
        private void CheckMethodForSwitches(MethodInfo methodInfo, IDictionary<string, string> switches)
        {
            ...
            foreach (KingSwitchesAttribute switchesAttribute in methodInfo.GetCustomAttributes(typeof(KingSwitchesAttribute), false))
            {
